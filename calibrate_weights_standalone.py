#!/usr/bin/env python3
"""
Script independiente para calibrar los pesos de las métricas

Implementa las métricas directamente sin depender de tester_lib.py
...
class StandaloneWeightCalibrator:
    """Clase independiente para calibrar los pesos de las métricas"""
    ...
    def evaluate_curves(self, curves: List[List[float]]) -> List[Dict]:
        ...
        for i, curve in enumerate(curves):
            ...
            try:
                # Calcular métricas usando implementación propia
    ...
    def _calculate_metrics(self, curve: List[float]) -> Dict:
        """Calcula todas las métricas para una curva"""
        n = len(curve)
        if n < 2:
            return self._get_invalid_metrics()
        
        # ---------- nº de trades (normalizado) -----------------------------------
        returns = [curve[i] - curve[i-1] for i in range(1, n)]
        ...
        # Normalización usando función logística suave
        trade_nl = 1.0 / (1.0 + math.exp(-(n_trades - min_trades) / (min_trades * 5.0)))
        ...
        max_dd = max(running_max[i] - curve[i] for i in range(n))
        ...
        # Normalización avanzada de rdd
        rdd_nl = 1.0 / (1.0 + math.exp(-(rdd - rdd_floor) / (rdd_floor * 5.0)))
        ...
        # Normalización avanzada del Calmar Ratio
        calmar_nl = 1.0 / (1.0 + math.exp(-(calmar_ratio - calmar_floor) / (calmar_floor * 65.0)))
        
        # ---------- linealidad y pendiente (normalizado) ---------------------------
        x = list(range(n))
        y = curve
        
        r2, slope = self._manual_linear_regression(x, y)
        if slope < 0.0:
            return self._get_invalid_metrics()
        
        slope_nl = min(1.0, 1.0 / (1.0 + math.exp(-(math.log1p(slope / n) - 0.001) / 0.003)))
    ...
    def _get_invalid_metrics(self) -> Dict:
        """Retorna métricas inválidas"""
        return {
            'trade_nl': -1.0,
            'rdd_nl': -1.0,
            'r2': -1.0,
    ...
    def _manual_linear_regression(self, x: List[float], y: List[float]) -> Tuple[float, float]:
...
def main():
    ...
    print("=== CALIBRADOR DE PESOS PARA TESTER (VERSIÓN INDEPENDIENTE) ===\n")
    
    # Crear instancia del calibrador
    calibrator = StandaloneWeightCalibrator()
