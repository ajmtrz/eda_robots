...
class StrategySearcher:
    ...
    def _compute_meta_labels_oof_reg(
        self,
        y_main: pd.Series,
        ...
        if self.debug:
            n_true = mask.sum()
            n_total = len(mask)
            print(f"🔍 DEBUG: _compute_meta_labels_oof_reg - {n_true}/{n_total} muestras con etiqueta meta=1.0 ({n_true/n_total:.2%})")

        return mask

    def _compute_main_oof_predictions_reg(
        self,
        X_main: pd.DataFrame,
        y_main: pd.Series,
        hp: Dict[str, Any],
