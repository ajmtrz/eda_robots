...
class StrategySearcher:
    ...
    def fit_final_models(self, main_data: pd.DataFrame,
        ...
        try:
            ...
            meta_intervals = None
            try:
                # Verificar que hay al menos dos clases en los datos de entrenamiento
                if len(np.unique(y_train_main)) < 2:
                    print("⚠️ ERROR: Todas las etiquetas del modelo principal son iguales")
                    return (-1.0, -1.0), None, None, None, None
                    
                if len(np.unique(y_train_meta)) < 2:
                    print("⚠️ ERROR: Todas las etiquetas del modelo meta son iguales")
                    return (-1.0, -1.0), None, None, None, None
